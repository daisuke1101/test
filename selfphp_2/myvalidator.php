...
class MyValidator {
    // エラーメッセージを格納するためのプライベート変数(配列)、クラス外からは不可視にすること
    private $_errors;

    // コンストラクタ
    public function __construct() {
        // プライベート変数$_errors配列の初期化
        $_errors = array();
    ...
    // 配列要素に含まれる文字エンコーディングをチェック(クラス外からは不可視)
    ...
    // 配列要素に含まれるNULL(ナル)バイトをチェック(クラス外からは不可視)
    ...
    public function intTypeCheck($value, $name) {
        if (trim($value) !== '') {
            if(!ctype_digit($value)) {
                $this->_errors[] = "{$name}は整数で指定してください。";
            }
        }
    }
    ...
    // プライベート変数_errors[]にエラーが含まれる場合にはリスト表示
